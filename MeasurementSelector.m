...
for idx = 1:length(PLOTSEL)
    ...
    if ~isempty(isBasicFreqFcn)
        ...
        if strcmp(plotSel, 'Band Relative Power')
            ylims = [0 1];
    ...
    elseif strcmp(plotSel, 'Trough Freq (Hz)')
        % peak freq
        fcn{idx} = @(Spec, EEG) troughFreqEpoch(Spec, EEG, bnd, BandTableHz);
        if isa(bnd,'char') | isa(bnd,'string')
            ylims = band2freqs(bnd, BandTableHz);
        else
            ylims = bnd;
...
function [Freq,times] = troughFreqEpoch(epoch_Spec, epoch_EEG, bnd, tbl)
% inspect -peakFreq across epochs (time)
    nchan = epoch_Spec(1).nbchan;
    PAFs = zeros(nchan, length(epoch_Spec), 2);
    for chan = 1:nchan
        PAFs(chan,:,1) = arrayfun(@(eegSpec) ...
            peakFreq(eegSpec.frequency1side, ...
                        max(eegSpec.powerSpectrum(chan,:))-1*(eegSpec.powerSpectrum(chan,:)), ...
            bnd, tbl), ...
            epoch_Spec);
        PAFs(chan,:,2) = arrayfun(@(eeg) ...
            mean([eeg.xmin, eeg.xmax]), epoch_EEG);
